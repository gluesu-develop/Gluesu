...
EXPORT_SYMBOL(gluesu_kpm_unload_module);

noinline NO_OPTIMIZE void gluesu_kpm_num(int *result)
{
    pr_info("kpm: Stub function called (gluesu_kpm_num).\n");
...
EXPORT_SYMBOL(gluesu_kpm_control);

noinline NO_OPTIMIZE void gluesu_kpm_version(char *buf, int bufferSize)
{
    pr_info("kpm: Stub function called (gluesu_kpm_version). "
            "buffer=%p\n",
            buf);
}
                               ...
                               unsigned long result_code)
{
    ...
    if (control_code == GLUESU_KPM_LOAD) {
        ...
        gluesu_kpm_num(&res);
    } else if (control_code == GLUESU_KPM_INFO) {
        char kernel_name_buffer[256];
        char buf[256];
        int size;
    ...
    } else if (control_code == GLUESU_KPM_LIST) {
        char buf[1024];
        int len = (int)arg2;
        ...
        gluesu_kpm_list((char *)&buf, sizeof(buf), &res);
    ...
    } else if (control_code == GLUESU_KPM_CONTROL) {
        ...
        gluesu_kpm_version((char *)&buffer, sizeof(buffer));
...
EXPORT_SYMBOL(gluesu_handle_kpm);
